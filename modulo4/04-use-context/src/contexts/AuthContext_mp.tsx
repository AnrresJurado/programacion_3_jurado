...
export function AuthProvider_mp({ children }: { children: React.ReactNode }) {
  ...
  async function login(email: string, _password: string) {
    ...
    if (email === 'error@test.com') {
      dispatch({ type: 'LOGIN_ERROR', message: 'Credenciales de chofer incorrectas' });
      return;
    }

    dispatch({
      type: 'LOGIN_SUCCESS',
      user: { id: 101, name: 'Andrés Jurado', email, role: 'Despachador' },
    });
  }
...
export function useAuth_mp(): AuthContextValue {
  ...
  if (!context) throw new Error('useAuth_mp debe usarse dentro de <AuthProvider_mp>');
  return context;
}