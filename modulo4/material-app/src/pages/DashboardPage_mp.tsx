...
export default function DashboardPage_mp() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('No se pudieron cargar los repuestos de flota. Verifica la conexión con la API.')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight={700} gutterBottom>
        Inventario de Repuestos y Unidades (_mp)
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        API Endpoint: /products/?page={page}&page_size={pageSize}
      ...
      {loading ? (
        ...
        <Alert severity="info">No hay repuestos registrados en el inventario de flota.</Alert>
      ) : view === 'cards' ? (
              ...
              <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                <CardMedia
                  ...
                  onError={(e) => {
                    ;(e.currentTarget as HTMLImageElement).src =
                      'https://placehold.co/400x140?text=Repuesto+Flota'
                  }}
                />
                <CardContent sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                    <Chip
                      label={p.is_active ? 'Disponible' : 'Agotado'}
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: 'grey.50' }}>
                <TableCell>Repuesto / Pieza</TableCell>
